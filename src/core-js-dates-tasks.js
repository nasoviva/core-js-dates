...
function getNextFriday(date) {
  ...
  const count = (5 - dayOfWeek + 7) % 7 || 7;
  currentDay.setUTCDate(count + currentDay.getUTCDate());
  return currentDay;
}
...
function formatDate(date) {
  ...
  const sec =
    new Date(date).getSeconds() > 9
      ? `${new Date(date).getSeconds()}`
      : `0${new Date(date).getSeconds()}`;
  return `${month}/${day}/${year}, ${hour}:${min}:${sec} ${period}`;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  const lastOf = new Date(year, month, 0);
  let count = 0;
  for (let i = firstOf; i <= lastOf; i.setUTCDate(i.getUTCDate() + 1)) {
    const day = i.getDay();
    if (day === 0 || day === 6) {
      count += 1;
    }
...
function getWeekNumberByDate(date) {
  ...
  const year = new Date(day.getFullYear(), 0, 1);
  const countOfDays = Math.floor((day - year) / (1000 * 60 * 60 * 24));
  return Math.ceil((countOfDays + 1) / 7);
}
